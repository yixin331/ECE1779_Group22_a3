...
def upload():
    ...
    if file and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS:
        ...
        response = None
        try:
            response = requests.post(url='http://35.173.213.171:5002/putImage', data=keyToSend, files=fileToSend).json()
...
def list_key(key):
    ...
    response = None
    try:
        response = requests.post(url='http://35.173.213.171:5002/getKey', data=keyToSend).json()
    except requests.exceptions.ConnectionError as err:
        webapp.logger.warning("Manager app loses connection")
...
def pop_up():
    response = requests.get(url='http://35.173.213.171:5002/pop_up').json()
    message = {}
